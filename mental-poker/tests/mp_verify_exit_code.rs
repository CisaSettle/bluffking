...
///
/// U40 (dual-AI OSS review): the committed regeneration path is the ignored
/// test `regenerate_sound_reenc_fixture` (phase4_server_blind.rs):
///
/// ```sh
...
/// ```
///
/// Key material comes from `OsRng`, so regenerated bytes differ each run — any
/// SOUND transcript satisfies this consumer; commit and review the diff. (The
/// sibling golden files here regenerate via `UPDATE_KAT_VECTORS=1` — see
/// `conformance_vectors.rs` and `emit_phase4_ec_kat_vectors`.)
